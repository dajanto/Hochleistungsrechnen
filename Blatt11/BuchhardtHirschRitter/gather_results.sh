...
while [ $i -le $line_count ]; do
    ...
    header=`echo 'NPROCS NNODES ILINES TIME DEVIATION'`
    cat <<EOF>${case}.dat
${header}
EOF
    ...
    sum=0.;	count=0; meant=-1; deviation=-1; square_sum=0
    for t in $times; do
    ...
    if [[ $sum > 0. ]]; then
        ...
        for t in $times; do
            square_sum=`echo "scale=${FPREC}; ${square_sum} + ((${t} - ${meant}) * (${t} - ${meant}))" | bc`
        done
	deviation=`echo "scale=${FPREC}; sqrt(${square_sum} / (${count} - 1)) " | bc`
    fi
    
    data="${procs} ${nodes} ${ilins} ${meant} ${deviation}"
    cat <<EOF>>${case}.dat
${data}
EOF
    echo ${data}
